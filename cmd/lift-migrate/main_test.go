...
func TestMapImport(t *testing.T) {
	...
	if !ok || got != "github.com/theory-cloud/apptheory/v4/pkg/limited" {
		t.Fatalf("mapImport base: got %q ok=%v", got, ok)
	}

	got, ok = mapImport("github.com/pay-theory/limited/strategies")
	if !ok || got != "github.com/theory-cloud/apptheory/v4/pkg/limited/strategies" {
...
`
	...
	if !strings.Contains(string(out), `github.com/theory-cloud/apptheory/v4/pkg/limited/strategies`) {
	...
	if !strings.Contains(string(updated), `github.com/theory-cloud/apptheory/v4/pkg/limited/strategies`) {
