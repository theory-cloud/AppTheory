...
| THR-4 | Non-reproducible builds / provenance drift | Artifacts differ per build environment or time; release tags do not correspond to source; tool versions drift. | COM-2, SEC-4, CMP-4 | `scripts/verify-builds.sh` |
...
| THR-9 | Evidence/doc drift (“paper security”) | Docs claim controls exist, but verifiers don’t run or evidence isn’t reproducible; threats are unmapped. | DOC-4, DOC-5, CMP-1..4 | `bash gov-infra/verifiers/gov-verify-rubric.sh` |
...
| THR-11 | Release lane invariant drift | Release branches, Release Please state, generated artifacts, or publisher recovery paths drift from the single valid cycle, making a stale or mutable release path mergeable. | CMP-4 | `bash gov-infra/verifiers/gov-verify-rubric.sh` |
