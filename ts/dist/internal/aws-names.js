...
export function kinesisStreamNameFromArn(arn) {
    const value = String(arn ?? "").trim();
    if (!value)
        return "";
    const parts = value.split(":");
...
export function snsTopicNameFromArn(arn) {
    const value = String(arn ?? "").trim();
    if (!value)
        return "";
    const parts = value.split(":");
...
export function eventBridgeRuleNameFromArn(arn) {
    const value = String(arn ?? "").trim();
    if (!value)
        return "";
...
export function dynamoDBTableNameFromStreamArn(arn) {
    const value = String(arn ?? "").trim();
    if (!value)
        return "";
    ...
    if (streamIdx >= 0)
        return after.slice(0, streamIdx);
