...
export function validateMicroVMLifecycleHookSpecs(hooks) {
...
export function validateMicroVMLifecycleStateLists(contract) {
...
export function validateMicroVMLifecycleTransitionSet(hookSpecs, transitions) {
...
export function validateMicroVMLifecycleFailureTransitions(transitions) {
    for (const state of [
...
export function validateMicroVMLifecycleAdapterContract(contract) {
...
export function isMicroVMRealLifecycleContractShape(contract) {
...
export function microVMRealLifecycleOnlyHook(hook) {
...
export function microVMRealLifecycleOnlyState(state) {
...
export function lifecycleContractValidationError(err, requestID) {
...
export function normalizeMicroVMLifecycleEvent(event) {
...
export function lifecycleErrorResult(event, state, error) {
    const result = {
        request_id: String(event.request_id ?? "").trim(),
        tenant_id: String(event.tenant_id ?? "").trim(),
        namespace: String(event.namespace ?? "").trim(),
...
export function buildLifecycleResult(event, state) {
    const result = {
...
export function requiredMicroVMLifecycleHooks() {
    return [
...
export function requiredMicroVMLifecycleStates() {
    return [
...
export function preStateForMicroVMHook(hook) {
    switch (hook) {
...
export function cloneMicroVMLifecycleContract(contract) {
    return {
...
export function cloneMicroVMLifecycleEvent(event) {
    const out = {
        request_id: event.request_id,
        tenant_id: event.tenant_id,
        namespace: event.namespace,
...
export function normalizeMicroVMLifecycleHook(hook) {
    return String(hook ?? "").trim();
}
export function normalizeMicroVMLifecycleState(state) {
    return String(state ?? "").trim();
}
export function lifecycleContractIndex(contract) {
    const hooks = new Map();
    for (const hook of contract.hooks) {
...
export function microVMTransitionSet(transitions) {
    const set = new Set();
    const list = [];
...
export function microVMNextState(transitions, from, hook) {
...
export function transitionKey(from, hook, to) {
    return `${from}\u0000${hook}\u0000${to}`;
}
