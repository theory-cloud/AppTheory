...
export function microVMControllerAuthDefaultsDeny(auth) {
    return (auth.required === true &&
        String(auth.default ?? "")
            .trim()
            .toLowerCase() === MICROVM_CONTROLLER_AUTH_DEFAULT_DENY);
}
export function normalizeMicroVMCommand(command) {
    const normalized = String(command ?? "").trim();
    if (normalized === "shell-token")
        return MicroVMCommand.ShellAuthToken;
    return normalized;
}
export function requiredMicroVMControllerCommands() {
    return [
...
export function realMicroVMControllerCommands() {
    return [
        MicroVMCommand.Run,
        MicroVMCommand.Get,
        MicroVMCommand.List,
...
export function isRequiredMicroVMCommand(command) {
    return requiredMicroVMControllerCommands().includes(command);
}
export function validMicroVMCommand(command) {
...
export function requiredMicroVMSessionRegistryContractFields() {
...
//# sourceMappingURL=controller-contract.js.map