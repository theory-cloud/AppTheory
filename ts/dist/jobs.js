...
export function newJobLedgerError(type, message) {
    return new JobLedgerError(type, message);
}
export function wrapJobLedgerError(cause, type, message) {
    return new JobLedgerError(type, message, cause);
}
function isTheorydbCode(err, code) {
    if (err instanceof TheorydbError)
        return String(err.code ?? "") === code;
    if (!err || typeof err !== "object")
        return false;
    const rec = err;
    return (String(rec["code"] ?? "").trim() === code ||
...
export function jobsTableName() {
    return (String(process.env[EnvJobsTableName] ?? "").trim() || defaultJobsTableName);
}
export function jobPartitionKey(jobId) {
    return `JOB#${jobId}`;
}
export function jobMetaSortKey() {
    return "META";
}
export function jobRecordSortKey(recordId) {
    return `REC#${recordId}`;
}
export function jobLockSortKey() {
    return "LOCK";
}
...
function normalizeTtlUnixSeconds(now, ttlSeconds, fallbackSeconds) {
...
export function sanitizeErrorEnvelope(env) {
    if (!env)
        return undefined;
    const message = sanitizeLogString(String(env.message ?? "")) || "unknown error";
...
export function defaultJobsConfig() {
    return {
...
function normalizeJobsConfig(config) {
    const base = defaultJobsConfig();
    const merged = { ...base, ...config };
    merged.tableName = String(merged.tableName ?? "").trim() || base.tableName;
    merged.defaultLeaseDurationMs = Math.floor(Number(merged.defaultLeaseDurationMs) || 0);
    merged.defaultIdempotencyTtlSeconds = Math.floor(Number(merged.defaultIdempotencyTtlSeconds) || 0);
    merged.defaultJobTtlSeconds = Math.floor(Number(merged.defaultJobTtlSeconds) || 0);
    merged.defaultRecordTtlSeconds = Math.floor(Number(merged.defaultRecordTtlSeconds) || 0);
    merged.defaultRequestResultTtlSeconds = Math.floor(Number(merged.defaultRequestResultTtlSeconds) || 0);
    return merged;
}
const jobLedgerModelName = "JobLedgerItem";
function jobLedgerModel(tableName) {
    return defineModel({
...
function requireStr(value, field) {
    ...
    if (!v)
        throw newJobLedgerError("internal_error", `missing field: ${field}`);
    return v;
}
...
export class DynamoJobLedger {
    ...
    async createJob(input) {
        ...
        const ttl = normalizeTtlUnixSeconds(now, input?.ttlSeconds, this._config.defaultJobTtlSeconds);
        const item = {
            pk,
            sk,
            job_id: jobId,
    ...
    async transitionJobStatus(input) {
        const jobId = requireNonEmpty(input?.jobId, "jobId");
        const toStatus = requireNonEmpty(input?.toStatus, "toStatus");
        ...
        try {
            ...
            throw wrapJobLedgerError(err, "internal_error", "failed to transition job status");
        }
    }
    async upsertRecordStatus(input) {
        const jobId = requireNonEmpty(input?.jobId, "jobId");
        ...
        const now = this._clock.now();
        const nowIso = now.toISOString();
        const ttl = normalizeTtlUnixSeconds(now, input?.ttlSeconds, this._config.defaultRecordTtlSeconds);
        try {
            const builder = this._theorydb.updateBuilder(jobLedgerModelName, {
                pk,
                sk,
            ...
            if (env)
                builder.set("error", env);
            else
                builder.remove("error");
            builder.returnValues("ALL_NEW");
            const out = await builder.execute();
            if (!out)
                throw newJobLedgerError("internal_error", "missing update result");
            ...
            throw wrapJobLedgerError(err, "internal_error", "failed to upsert record status");
        }
    }
    async acquireLease(input) {
        ...
        try {
            ...
            throw wrapJobLedgerError(err, "internal_error", "failed to acquire lease");
        }
    }
    async refreshLease(input) {
        ...
        try {
            ...
            throw wrapJobLedgerError(err, "internal_error", "failed to refresh lease");
        }
    }
    ...
    async createIdempotencyRecord(input) {
        ...
        const now = this._clock.now();
        const nowIso = now.toISOString();
        const ttl = normalizeTtlUnixSeconds(now, input?.ttlSeconds, this._config.defaultIdempotencyTtlSeconds);
        const item = {
            pk,
            sk,
            job_id: jobId,
        ...
        try {
            ...
            if (!isConditionalCheckFailed(err)) {
                throw wrapJobLedgerError(err, "internal_error", "failed to create idempotency record");
            }
        }
        try {
            const existing = await this._theorydb.get(jobLedgerModelName, { pk, sk });
            const req = toJobRequest(existing);
            const outcome = req.status === "COMPLETED"
                ? "already_completed"
                : "already_in_progress";
            return { request: req, outcome };
        }
        catch (err) {
            throw wrapJobLedgerError(err, "internal_error", "failed to load existing idempotency record");
        }
    }
    async completeIdempotencyRecord(input) {
        ...
        const now = this._clock.now();
        const nowIso = now.toISOString();
        const ttl = normalizeTtlUnixSeconds(now, input?.ttlSeconds, this._config.defaultRequestResultTtlSeconds);
        try {
            const builder = this._theorydb.updateBuilder(jobLedgerModelName, {
                pk,
                sk,
            ...
            if (result)
                builder.setIfNotExists("result", null, result);
            const env = sanitizeErrorEnvelope(input?.error);
            if (env)
                builder.setIfNotExists("error", null, env);
            builder.returnValues("ALL_NEW");
            const out = await builder.execute();
            if (!out)
                throw newJobLedgerError("internal_error", "missing update result");
            ...
            throw wrapJobLedgerError(err, "internal_error", "failed to complete idempotency record");
        }
    }
}
