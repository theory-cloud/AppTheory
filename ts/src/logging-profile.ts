...
import type { LogRecord, ObservabilityHooks } from "./app.js";
...
export class ProfileLogger implements StructuredLogger {
  private readonly root: ProfileLogger;
  private readonly config: LoggingProfileConfig;
  private readonly environment: Record<string, string>;
  private readonly writer: ((line: string) => void) | null;
  private readonly sanitizer: LoggingProfileSanitizer;
  private readonly clock: () => Date;
  private readonly context: ProfileLoggerContext;

  private closed = false;
  private readonly profileEntries: Record<string, unknown>[] = [];
  private entriesLogged = 0;
  private lastError = "";

  constructor(
    config: LoggingProfileConfig,
    options: ProfileLoggerOptions = {},
  ) {
  ...
  private static fromRoot(
    root: ProfileLogger,
    context: ProfileLoggerContext,
  ): ProfileLogger {
    const logger = Object.create(ProfileLogger.prototype) as ProfileLogger;
    const mutable = logger as unknown as Record<string, unknown>;
  ...
  private clone(overrides: Partial<ProfileLoggerContext>): ProfileLogger {
  ...
  private log(level: string, message: string, fieldSets: LogFields[]): void {
    if (this.root.closed) return;
    const merged: LogFields = { ...this.context.fields };
    for (const fields of fieldSets) Object.assign(merged, fields);

    const event: LoggingProfileEvent = {
    ...
    try {
      const encoded = encodeLoggingProfileEventWithSanitizer(
        this.config,
        this.environment,
        event,
        this.sanitizer,
...
export function encodeLoggingProfileEvent(
  config: LoggingProfileConfig,
  environment: Record<string, string> | undefined,
  event: LoggingProfileEvent,
): Record<string, unknown> {
  return encodeLoggingProfileEventWithSanitizer(
    config,
    environment,
    event,
    sanitizeFieldValue,
...
export function encodeLoggingProfileEventWithSanitizer(
  config: LoggingProfileConfig,
  environment: Record<string, string> | undefined,
  event: LoggingProfileEvent,
  sanitizer: LoggingProfileSanitizer | undefined,
): Record<string, unknown> {
  validateLoggingProfile(config);
  ...
  putProfileField(
    out,
    timestampField(config),
    formatProfileTimestamp(event.timestamp, config.encoding?.timestamp_format),
    sanitizerFn,
  );
  putProfileField(
    out,
    levelField(config),
    profileLevel(config, event.level),
    sanitizerFn,
  );
  putProfileField(
    out,
    messageField(config),
    sanitizeLogString(String(event.message ?? "")),
    sanitizerFn,
  );
  ...
  if (missing.length > 0) {
    throw new Error(
      `logging profile required fields missing: ${missing.join(", ")}`,
    );
  }
  return out;
}

export function hooksFromProfileLogger(
  config: LoggingProfileConfig,
  options: ProfileLoggerOptions = {},
): { hooks: ObservabilityHooks; logger: ProfileLogger } {
  const logger = new ProfileLogger(config, options);
  return { hooks: hooksFromLogger(logger), logger };
}

export function hooksFromLogger(
  logger: StructuredLogger | null | undefined,
): ObservabilityHooks {
  if (!logger) return {};
  return {
    log(record: LogRecord): void {
      ...
      if (record.level === "error") scoped.error(record.event, fields);
      else if (record.level === "warn") scoped.warn(record.event, fields);
      else if (record.level === "debug") scoped.debug(record.event, fields);
      else scoped.info(record.event, fields);
    },
  };
}

function addIfPresent(fields: LogFields, key: string, value: unknown): void {
  if (trimString(value)) fields[key] = value;
}

function timestampField(config: LoggingProfileConfig): string {
  const field = trimString(config.encoding?.timestamp_field);
  if (field) return field;
  return mappedFieldOrDefault(config, "timestamp", "timestamp");
}

function levelField(config: LoggingProfileConfig): string {
  const field = trimString(config.encoding?.level_field);
  if (field) return field;
  return mappedFieldOrDefault(config, "severity", "level");
}

function messageField(config: LoggingProfileConfig): string {
  const field = trimString(config.encoding?.message_field);
  if (field) return field;
  return mappedFieldOrDefault(config, "message", "message");
}

function mappedFieldOrDefault(
  config: LoggingProfileConfig,
  canonical: string,
  fallback: string,
): string {
...
function putCanonicalMappedField(
  ...
  putProfileField(
    out,
    mappedFieldOrDefault(config, canonical, canonical),
    value,
    sanitizer,
  );
}

function putProfileField(
  out: Record<string, unknown>,
  field: string,
  value: unknown,
  sanitizer: LoggingProfileSanitizer,
  ...
  if (!key || isZeroProfileValue(value)) return;
  out[key] = sanitizer(key, value);
}

function putProfileRawString(
  out: Record<string, unknown>,
  field: string,
...
function formatProfileTimestamp(
  ...
  if (normalizedFormat === "rfc3339") return iso.replace(/\.\d{3}Z$/, "Z");
  return iso.replace(/\.(\d{3})Z$/, (_match, millis: string) => {
...
function applyStaticEnrichment(
  out: Record<string, unknown>,
  config: LoggingProfileConfig,
  environment: Record<string, string> | undefined,
  sanitizer: LoggingProfileSanitizer,
  ...
  for (const field of sortedKeys(config.enrichment?.static)) {
    const value = resolveStaticEnrichmentValue(
      config.enrichment?.static?.[field],
      environment,
    );
...
function resolveStaticEnrichmentValue(
  value: string | undefined,
  environment: Record<string, string> | undefined,
): string {
...
function applyContextEnrichment(
  ...
  for (const field of sortedKeys(config.enrichment?.context)) {
    const value = contextSourceValue(
      config.enrichment?.context?.[field],
      event,
    );
    putProfileField(out, field, value, sanitizer);
  }
...
function applyErrorCapture(
  ...
  if (config.error_capture?.include_stack_trace) {
    const field =
      trimString(config.error_capture.stack_trace_field) ||
  ...
  if (stackHashField && trimString(event.error?.stack_trace)) {
    putProfileField(
      out,
      stackHashField,
      profileStackHash(event.error?.stack_trace ?? ""),
      sanitizer,
    );
  }
}

function profileStackHash(stackTrace: string): string {
  return `sha256:${createHash("sha256").update(stackTrace).digest("hex")}`;
}

function applySafeEventFields(
  out: Record<string, unknown>,
  ...
  for (const key of sortedKeys(fields)) {
    ...
    if (!isAllowedProfileEventField(trimmed)) continue;
    if (Object.prototype.hasOwnProperty.call(out, trimmed)) continue;
    putProfileField(out, trimmed, fields?.[key], sanitizer);
  }
}

function isAllowedProfileEventField(field: string): boolean {
...
function isZeroProfileValue(value: unknown): boolean {
...
function applyKnownProfileFieldsToEvent(
  event: LoggingProfileEvent,
  fields: LogFields,
): void {
  event.normalized_message =
...
function firstStringField(fields: LogFields, ...names: string[]): string {
...
function stringField(fields: LogFields, name: string): string {
  const value = fields[name];
  if (value === null || value === undefined) return "";
  return typeof value === "string" ? value : String(value);
}

function numberField(fields: LogFields, name: string): number {
  const value = fields[name];
