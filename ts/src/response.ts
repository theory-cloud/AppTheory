...
async function* normalizeHTMLChunks(
  ...
  if (Symbol.asyncIterator in chunks) {
    for await (const chunk of chunks as AsyncIterable<StreamableHTMLChunk>) {
      yield toBuffer(chunk);
    }
    return;
  }
  for (const chunk of chunks as Iterable<StreamableHTMLChunk>) {
    yield toBuffer(chunk);
  }
}
...
function parseForwardedHeader(value: unknown): { proto: string; host: string } {
  const raw = String(value ?? "").trim();
...
function firstCommaToken(value: unknown): string {
...
export function originURL(headers: Headers): string {
  ...
  let proto =
    firstHeaderValue(h, "cloudfront-forwarded-proto") ||
...
function parseCloudFrontViewerAddress(value: unknown): string {
