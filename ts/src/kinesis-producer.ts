...
const KINESIS_PUT_RECORDS_INVALID_MESSAGE =
  "apptheory: kinesis put-records result invalid";
 ...
 */
export function createKinesisJsonRecord(
  options: KinesisJsonRecordOptions,
): KinesisJsonRecord {
  const partitionKey = normalizeKinesisPartitionKey(options.partitionKey);
  const explicitHashKey = normalizeKinesisExplicitHashKey(
    options.explicitHashKey,
  );
  const data = encodeKinesisJsonPayload(options.payload);
  const record: KinesisJsonRecord = {
    partition_key: partitionKey,
    data,
    safe_summary: kinesisJsonRecordSafeSummary(
      partitionKey,
      data.byteLength,
      explicitHashKey,
    ),
  };
  if (explicitHashKey) record.explicit_hash_key = explicitHashKey;
  return record;
}
 ...
 */
export function reportKinesisPutRecordsFailures(
  ...
  if (records.length !== results.length) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: records/results length mismatch records=${records.length} results=${results.length}`,
    );
  }
  if (records.length > KINESIS_MAX_PUT_RECORDS_RECORDS) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: record count ${records.length} exceeds ${KINESIS_MAX_PUT_RECORDS_RECORDS}`,
    );
  }

  const failures: KinesisPutRecordsFailure[] = [];
  for (const [index, record] of records.entries()) {
    const normalizedRecord = normalizeKinesisReportRecord(record, index);
    const result = normalizeKinesisPutRecordsResultRecord(
      results[index] ?? {},
      index,
    );
    if (!result.error_code) continue;
...
function encodeKinesisJsonPayload(payload: unknown): Uint8Array {
  const json = stableJsonStringify(payload);
  const data = Buffer.from(json, "utf8");
  if (data.byteLength === 0) {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: empty json payload`,
    );
  }
  if (data.byteLength > KINESIS_MAX_RECORD_DATA_BYTES) {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: json payload size ${data.byteLength} exceeds ${KINESIS_MAX_RECORD_DATA_BYTES}`,
    );
  }
  return Uint8Array.from(data);
}

function normalizeKinesisPartitionKey(value: string): string {
  const partitionKey = String(value ?? "").trim();
  if (!partitionKey) {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: partition key is required`,
    );
  }
  const byteLength = Buffer.byteLength(partitionKey, "utf8");
  if (byteLength > KINESIS_MAX_PARTITION_KEY_BYTES) {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: partition key length ${byteLength} exceeds ${KINESIS_MAX_PARTITION_KEY_BYTES} bytes`,
    );
  }
  return partitionKey;
}

function normalizeKinesisExplicitHashKey(value: string | undefined): string {
  let explicitHashKey = String(value ?? "").trim();
  if (!explicitHashKey) return "";
  if (!/^\d+$/u.test(explicitHashKey)) {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: explicit hash key must be decimal digits`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: explicit hash key exceeds Kinesis hash key range`,
    );
  }
  return explicitHashKey;
}

function kinesisJsonRecordSafeSummary(
  partitionKey: string,
...
function normalizeKinesisReportRecord(
  record: KinesisJsonRecord,
  index: number,
): KinesisJsonRecord {
  let partitionKey: string;
  let explicitHashKey: string;
  try {
  ...
  if (!(record.data instanceof Uint8Array) || record.data.byteLength === 0) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: empty record data at index ${index}`,
    );
  }
  if (record.data.byteLength > KINESIS_MAX_RECORD_DATA_BYTES) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: record data size ${record.data.byteLength} exceeds ${KINESIS_MAX_RECORD_DATA_BYTES} at index ${index}`,
    );
  }
  const normalized: KinesisJsonRecord = {
    partition_key: partitionKey,
    data: record.data,
    safe_summary: kinesisJsonRecordSafeSummary(
      partitionKey,
  ...
  if (explicitHashKey) normalized.explicit_hash_key = explicitHashKey;
  return normalized;
}

function normalizeKinesisPutRecordsResultRecord(
  result: KinesisPutRecordsResultRecord,
  index: number,
): Required<KinesisPutRecordsResultRecord> {
  const normalized = {
  ...
  if (!normalized.error_code && normalized.error_message) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: error message without error code at index ${index}`,
    );
  }
  if (
    Buffer.byteLength(normalized.error_code, "utf8") >
    KINESIS_MAX_ERROR_CODE_BYTES
  ) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: error code too long at index ${index}`,
    );
  }
  if (hasUnsafeErrorCodeRune(normalized.error_code)) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: unsafe error code at index ${index}`,
    );
  }
  if (
    Buffer.byteLength(normalized.error_message, "utf8") >
    KINESIS_MAX_ERROR_MESSAGE_BYTES
  ) {
    throw new Error(
      `${KINESIS_PUT_RECORDS_INVALID_MESSAGE}: error message too long at index ${index}`,
    );
  }
  return normalized;
}

function hasUnsafeErrorCodeRune(value: string): boolean {
...
function kinesisPutRecordsFailure(
  index: number,
  record: KinesisJsonRecord,
  result: Required<KinesisPutRecordsResultRecord>,
): KinesisPutRecordsFailure {
  const errorMessageByteLength = Buffer.byteLength(
    result.error_message,
    "utf8",
  );
  const failure: KinesisPutRecordsFailure = {
    index,
...
function kinesisPutRecordsFailureSafeLog(
...
function kinesisPutRecordsFailureReportSummary(
  recordCount: number,
...
function stableJsonStringifyValue(
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      throw new Error(
        `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: json encode: non-finite number`,
      );
    }
  ...
  if (typeof value !== "object") {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: json encode: unsupported value`,
    );
  }

  if (seen.has(value)) {
    throw new Error(
      `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: json encode: circular value`,
    );
  }
  seen.add(value);
  try {
    ...
    if (!isPlainRecord(value)) {
      throw new Error(
        `${KINESIS_JSON_RECORD_INVALID_MESSAGE}: json encode: unsupported object`,
      );
    }
