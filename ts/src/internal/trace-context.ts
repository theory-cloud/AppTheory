...
export function extractTraceIdFromHeaders(
  ...
  const traceparent = traceIdFromTraceparent(
    firstNonEmptyHeaderValue(headers, TRACEPARENT_HEADER),
  );
  if (traceparent) return traceparent;
  return traceIdFromXAmznTraceId(
    firstNonEmptyHeaderValue(headers, X_AMZN_TRACE_ID_HEADER),
  );
}
...
function traceIdFromTraceparent(value: string): string {
  ...
  const traceId = String(parts[1] ?? "")
    .trim()
    .toLowerCase();
  if (!/^[0-9a-f]{32}$/.test(traceId) || /^0+$/.test(traceId)) return "";
  return traceId;
}

function traceIdFromXAmznTraceId(value: string): string {
...
function validXRayRoot(root: string): boolean {
  ...
  if (!/^[0-9a-fA-F]{8}$/.test(epoch)) return false;
  if (!/^[0-9a-fA-F]{24}$/.test(unique)) return false;
