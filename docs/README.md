...
The hosted Pages site at <https://apptheory.theorycloud.ai/> is published from the `staging` integration branch on
every push and re-deployed from the exact release tag automatically whenever a stable release publishes (release.yml
dispatches pages.yml with the tag). The version pill and install commands are stamped from the release tag at build
time; `workflow_dispatch` from a deployment-policy-allowed ref (the `staging` branch or a `v*` tag) re-deploys on
demand. Treat it as the integration documentation line, not as proof that a stable release has completed.
