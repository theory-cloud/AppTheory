...
- Go: `pkg/sanitization`
  - `SanitizeLogString`, `SanitizeFieldValue`, `SanitizeJSON`, `SanitizeXML`
...
- Python: `py/src/apptheory/sanitization.py` (exported from `apptheory`)
  - `sanitize_log_string`, `sanitize_field_value`, `sanitize_json`, `sanitize_xml`
...
- If you must log JSON payloads (e.g. event envelopes), log `sanitize_json(...)` / `SanitizeJSON(...)` output.
