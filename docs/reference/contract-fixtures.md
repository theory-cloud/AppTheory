...
description: "The 252 contract fixtures: Go, TypeScript, and Python execute all MCP, OAuth, and objectstore fixtures." # apptheory-fixture-count: 252
---

# Contract Fixtures

AppTheory ships **252 contract test fixtures** in `contract-tests/fixtures/`. <!-- apptheory-fixture-count: 252 --> Go, TypeScript, and Python execute the full fixture corpus on every commit, including the `tier: mcp` SP09 runtime contracts, the `tier: oauth` SP12 protected-resource, bearer, DCR, and PKCE contracts, and the `tier: objectstore` SP13 bounded object-store contract.
...
The 252 fixtures span these behavior areas (counts approximate; see `contract-tests/fixtures/` for the canonical inventory). Go, TypeScript, and Python execute all 252 fixtures, including MCP, OAuth, and objectstore. <!-- apptheory-fixture-count: 252 -->
...
This validates the fixture schema for all 252 files and runs the Go, TypeScript, and Python runners against the same fixture tree. `make rubric` runs this gate as part of the full repo check, alongside lint, build, API snapshots, and example synthesis.
