...
description: The 164 shared fixtures that arbitrate behavior across Go, TypeScript, and Python. # apptheory-fixture-count
---

# Contract Fixtures

AppTheory ships **164 contract test fixtures** in `contract-tests/fixtures/` <!-- apptheory-fixture-count --> that define the language-neutral behavior every runtime must produce. The Go, TypeScript, and Python runtimes are each independently verified against the same fixture corpus on every commit.
...
The 164 fixtures span these behavior areas (counts approximate; see `contract-tests/fixtures/` for the canonical inventory): <!-- apptheory-fixture-count -->
