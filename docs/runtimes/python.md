...
The Python runtime passes the 183 shared non-MCP contract fixtures on every commit. <!-- apptheory-fixture-count: 194 --> The runner loads the full 194-fixture tree and reports `skipped=11 mcp future-runtime fixtures` for the SP09 MCP tier until SP11 adds the Python MCP runtime. That skip is intentional staging, not parity proof. Tests live under `py/tests/` and are exercised by `./scripts/verify-python-tests.sh` and `make rubric`.
