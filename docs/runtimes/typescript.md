...
The TypeScript runtime is an independent implementation of the AppTheory contract — not a port of the Go runtime. It executes all [235 contract fixtures](../reference/contract-fixtures.md), including the SP09 MCP fixture tier for JSON-RPC, registries, sessions, Streamable HTTP, resumable SSE, task stores, the SP12 OAuth tier, and the SP13 objectstore tier. <!-- apptheory-fixture-count: 235 -->
...
The TypeScript runtime passes all 235 contract fixtures on every commit. <!-- apptheory-fixture-count: 235 --> The runner loads and executes the full 235-fixture tree, including the SP09 MCP tier and SP13 objectstore tier; the `ts/dist/` build output is checked in and gated by `make rubric`.
